...
############ EDITED funcction
def project_decode_latency(costs, per_layer_compute_timing,  prompt_len, gen_len):

    #per_layer_compute_timing: tensor parallel computation time for each layer

    decode_costs = costs[1:]
    tp_layer_costs = per_layer_compute_timing[1:]

    assert len(decode_costs) == len(tp_layer_costs)

    if gen_len / prompt_len < 0.1:
        warmup = 2
        decode_latency = (sum(decode_costs[:warmup]) + np.mean(decode_costs[warmup:]) * (gen_len - 1 - warmup))
        decode_compute_latency = (sum(tp_layer_costs[:warmup]) + np.mean(tp_layer_costs[warmup:]) * (gen_len - 1 - warmup))
    ...
    return decode_latency, decode_compute_latency
