...
# this variable for only execution_time_measurement, remove later
compute_time=list()
...
def init_weight_list(weight_specs, policy, env):
    ...
    for i in range(len(weight_specs)):
        ...
        ############# EDITED
        #print(f' weight specs:{weight_specs[i]}')
        #dist.barrier()
        #####
        ...
        ###### EDITED
        #print(f' reached here. compress: {compress}, pin_memory: {pin_memory}')
        #print(f'       home.dev: {home.dev}')
        #dist.barrier()
        ######
        
        if not compress:
            #print(f'  if not compress: shape:{shape}, dtype:{dtype}, pin_mem:{pin_memory}')
            weight = home.allocate(shape, dtype, pin_memory=pin_memory)
            ######## EDITED
            #print(f'   weight is completed')
            #dist.barrier()
            #######
            ...
            if DUMMY_WEIGHT not in filename:
                ...
                ########### EDITED
                #print(f'    shape:{shape}, dtype:{dtype}, weight: {weight}, file_name:{filename}, weights_specs[i][2]: {weight_specs[i][2]}')
                ...
                ########### EDITED
                #print(' load_from_np_file')
                #dist.barrier()
                ######### 

            else:
                ########### EDITED
                #print(f'    shape:{shape}, dtype:{dtype}, weight: {weight}')
                #dist.barrier()
                ###########
...
class InputEmbed:
    def __init__(self, config, env, policy, rank):
        self.config = config
        self.env = env
        self.policy = policy
    ...
    def init_weight(self, weight_home, path):
        ...
        print(f'CALLED INPUT WEIGHT\n\n\n     InputEmbed path original:{path}')
        ...
        ############## Edited
        path = os.path.join(path, f"rank_{self.rank}/")
        print(f'called input, path:{path}')
        
        weight_specs = [
            # w_token
            ((v, h), dtype, path + "decoder.embed_tokens.weight.npy"),
            # w_pos
            ((s + 2, h), dtype, path + "decoder.embed_positions.weight.npy"),
        ]
        weights = init_weight_list(weight_specs, self.policy, self.env)


        print('\n\n FINISHED init_weight for INPUT_EMBED\n\n')
        #dist.barrier()
        ##############
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        #### EDITED here to get computation time

        start_time = time.perf_counter()
        
        #print('called InputEmbed')
        ...
        compute_time.append(embed_time)
...
class OutputEmbed:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        ########### EDITED
        start_time = time.perf_counter()
        #print('called OutputEmbed')

        h = self.compute.opt_output_embed(h, w_ln, b_ln, w_token, donate,
            self.task.do_sample, self.task.temperature)
        
        end_time = time.perf_counter()
        output_forward_time = end_time - start_time

        compute_time.append(output_forward_time)
...
class SelfAttention:
    ...
    def init_weight(self, weight_home, path):
        ...
        #print(f'self atten path:{path}')

        weight_specs = [
            # w_q
            (( int(h/self.world_size), h), dtype, path + ".q_proj.weight.npy"),
            # b_q
            (( int(h/self.world_size),), dtype, path + ".q_proj.bias.npy"),
            # w_k
            (( int(h/self.world_size), h), dtype, path + ".k_proj.weight.npy"),
            # b_k
            (( int(h/self.world_size),), dtype, path + ".k_proj.bias.npy"),
            # w_v
            (( int(h/self.world_size), h), dtype, path + ".v_proj.weight.npy"),
            # b_v
            (( int(h/self.world_size),), dtype, path + ".v_proj.bias.npy"),
            # w_out
            ((h, int(h/self.world_size)), dtype, path + ".out_proj.weight.npy"),
            # b_out
            ((h,), dtype, path + ".out_proj.bias.npy"),
            # w_ln
            ((h,), dtype, path + "_layer_norm.weight.npy"),
            # b_ln
            ((h,), dtype, path + "_layer_norm.bias.npy"),
        ]


        weights = init_weight_list(weight_specs, self.policy, self.env)
        #print('self attention init-weights-done')
        weight_home.store(weights)
        #print('SelfAttention weight done.\n')
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if i == 0:  # prefill
            ...
            '''
            #compute_time.append(prefill_time)
            
            '''

            h, new_k_cache, new_v_cache, prefill_time = self.compute.mha_tp(h, mask, w_q, b_q,
                w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head, donate,
            ...
            '''
            ############################ EDIT
            #print('---Calling mha_gen/mha_gen_2')

            h, new_k_cache, new_v_cache, compute_mha_gen_time = self.compute.mha_gen_2(h, mask, w_q,
                b_q, w_k, b_k, w_v, b_v, w_out, b_out, w_ln, b_ln, n_head,
                k_cache, v_cache, donate, self.policy.attn_sparsity,
                self.policy.compress_cache, self.policy.comp_cache_config)
            # only for mha_gen_2
            # torch.cuda.synchronize()
            compute_time.append(compute_mha_gen_time)
            '''
        ...
        ############################# EDITED
        #print(f'called self attention:{h.shape}, max:{h.data.max()}, min:{h.data.min()} mean:{h.data.mean()}')
        #print(f'w_q:{w_q.data.shape}, w_out:{w_out.data.shape}, mask:{mask.data.shape}, k_cache:{new_k_cache.data.shape}')
...
class MLP:
    ...
    def init_weight(self, weight_home, path):
        ...
        #### this weights gets transposed in Linear layer
        weight_specs = [
            # wi
            (( int((4 * h)/self.world_size), h), dtype, path + "fc1.weight.npy"),
            # bi
            (( int((4 * h)/self.world_size),), dtype, path + "fc1.bias.npy"),
            # wo
            ((h, int((4 * h)/self.world_size)), dtype, path + "fc2.weight.npy"),
            # bo
            ((h,), dtype, path + "fc2.bias.npy"),
            # w_ln
            ((h,), dtype, path + "final_layer_norm.weight.npy"),
            # b_ln
            ((h,), dtype, path + "final_layer_norm.bias.npy"),
        ]

        #dist.barrier()
        #print(f'\n\n YYYYYYY rank:{self.rank}, {weight_specs} \n policy:{self.policy} env:{self.env}')
        
        weights = init_weight_list(weight_specs, self.policy, self.env)
        #dist.barrier()
        #print("\n\n      init_weight_list loading done for all mlp.\n") 
        weight_home.store(weights)
        #dist.barrier()
        #print("\n\n      weight loading done for all mlp.\n")  
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.mlp_TP(h, wi, bi, wo, bo, w_ln, b_ln, donate)

        #print('called mlp')
        #h, compute_mlp_time = self.compute.mlp_2(h, wi, bi, wo, bo, w_ln, b_ln, donate)
        #torch.cuda.synchronize()

        # only for mlp_2
        #compute_time.append(compute_mlp_time)
        #hval=h.data 
        #print(f'---------------------Hidden type:{type(hidden)}')
        #dist.all_reduce(hval, op = dist.ReduceOp.SUM)
        
        
        hidden.val = h#TorchTensor.create_from_torch(hval, self)
...
class OptLM:
    def __init__(self,
        ...
        ########### Edtied, self.rank
        layers.append(InputEmbed(self.config, self.env, self.policy, self.rank))
        ...
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                ...
                layers.append(SelfAttention(self.config, self.env, self.policy, i, self.rank, self.world_size))
                #layers.append(MLP(self.config, self.env, self.policy, i))

                layers.append(MLP(self.config, self.env, self.policy, i, self.rank, self.world_size))
            else:
        ...
        ########### EDITED
        #print(f'OptLM, finished all layer appending, rank:{self.rank}')
        #print(f'OptLM, env:{env}')

        self.init_all_weights()
        #print(f' FINIshED init_all_weights, rank:{self.rank}')
        ###########
    ...
    def init_weight(self, j):
        ...
        if self.rank==0:
            ...
            #print(f'dowload_path:{check_path}, rank path:{self.rank_path}')
            if not os.path.exists(check_path) and DUMMY_WEIGHT not in check_path:
                download_opt_weights(self.config.name, self.path, self.world_size, self.rank_path)
    ...
    def generate(self,
        ...
        ################
        print(f'overlap:{overlap}, gpu_batch_size:{gpu_batch_size}, num_gpu_batches:{num_gpu_batches}')
        ...
        if debug_mode is None:
            if not overlap:
                ...
                #####
                print(f'calling generation_loop_normal()')
                self.generation_loop_normal()
            else:
                # Overlap I/O and compute
                if num_gpu_batches == 1:
                    ########
                    print('calling generation_loop_overlap_single_batch()')
                    self.generation_loop_overlap_single_batch()
                else:
                    ########
                    print(f'calling generation_loop_overlap_multibatch()')
                    self.generation_loop_overlap_multi_batch()
        elif debug_mode == "fewer_batch":
            # Run fewer layeres and batches for debugging
            if num_gpu_batches == 1:
                print('calling Deubg mode. generation_loop_Debug_single_batch()')
                self.generation_loop_debug_single_batch()
            else:
                print('calling Debug mode. generation_loop_debug_multi_batch()')
    ...
    def generation_loop_normal(self):
        for i in range(self.execute_gen_len):
            print(f'i:{i} num_layers:{self.num_layers}, execute_gen_len:{self.execute_gen_len}')
    ...
    def generation_loop_overlap_single_batch(self):
        ...
        for i in range(self.execute_gen_len):
            ...
            for j in range(self.num_layers):
                ...
                ################## EDITED
                #print('JUST before compute_layer')
                self.compute_layer(i, j, 0)
                #print('JUST after compute_layer')
    ...
    def generation_loop_debug_single_batch(self):
        ...
        # Convert "decoding_gpu_batch" timer to "generate" timer
        print(f'BATCH COST inside multibatch: cost len= {len(costs)}')
        batch_cost = np.mean(timers("decoding_gpu_batch").costs[10:])
        for i in range(self.execute_gen_len):
            if i == 0:
    ...
    ########## EDited
    # commented out, this automatically handles the manual deletion
...
def init_processes(rank, backend, args, world_size):
    ...
    try:
        ...
        run_flexgen(rank, world_size, args)

    finally:

        dist.destroy_process_group()

def run_flexgen(rank, world_size, args):
    ...
    gpu = TorchDevice("cuda:{}".format(rank))
    cpu = TorchDevice("cpu")
    ...
    # ######### Edited
    model = OptLM(opt_config, env, args.path, policy, rank, world_size, args.rank_path)
    ...
    try:
        ...
        print('\ntesting start, warm up done\n')
        
        print(f'cut_gen_len:{cut_gen_len}, gen_len:{args.gen_len}')
        print(f'clearning compute_time list to remove waramp ups')
        #print('Compute time:', compute_time)
        compute_time.clear()
         
        #assert not compute_time, "The list is not empty!"
        ...
        ############### Edited
        # compute cost contains each layer computation time.
        #num_layers_per_token = int(len(compute_time)/args.gen_len)
        
        #gen_len_timing = np.array(compute_time).reshape(args.gen_len, num_layers_per_token)
        #per_layer_compute_timing = list(np.sum(gen_len_timing, axis = 1))
        #compute_latency = np.mean(per_layer_compute_timing)
         
        #print(f'num_layers_per_token:{num_layers_per_token}, gen_len_timing shape:{gen_len_timing.shape}')
        #print(f'per_layer_timing: {per_layer_compute_timing}')
        #print(f'BATCH COST inside multibatch: cost len= {len(costs)}, cost mean={np.mean(costs)}')
        #print(f'costs:{costs}')
        #print(f'compute time len:{len(compute_time)}, mean:{np.mean(compute_time)}')
        #print('compute time :', compute_time.shape)
    ...
    #assert len(costs) == len(per_layer_compute_timing) , "error in total layer timing computation"

    #tp_compute_prefill_latency = per_layer_compute_timing[0]
    
    #print(f'tp compute only prefill latency:{tp_compute_prefill_latency}')
    ######################### EDITED
    
    #print(f'tp full prefill latency:{prefill_latency}')
    #computed_prefill_latency = np.sum()

    prefill_throughput = num_prompts * prompt_len / prefill_latency
    ############### EDITED
    #tp_compute_prefill_throughput = num_prompts * prompt_len / tp_compute_prefill_latency

    if cut_gen_len:  # project latency of cut_gen_len to gen_len

        ############## EDITED
        #decode_latency, tp_compute_decode_latency = project_decode_latency(costs, per_layer_compute_timing, prompt_len, gen_len)
        ############# original
        decode_latency = project_decode_latency(costs, prompt_len, gen_len)
    else:
        decode_latency = sum(costs[1:])
        ############## EDITED
        #tp_compute_decode_latency=sum(per_layer_compute_timing[1:])
    

    decode_throughput = num_prompts * (gen_len - 1) / max(decode_latency, 1e-10)
    ####### EDITED
    #tp_compute_decode_throughput = num_prompts * (gen_len - 1) / max(tp_compute_decode_latency, 1e-10)

    num_generated_tokens = num_prompts * gen_len
    total_latency = prefill_latency + decode_latency
    
    ####### EDITED
    #tp_compute_total_latency = tp_compute_prefill_latency + tp_compute_decode_latency

    ######## throughput
    total_throughput = num_generated_tokens / total_latency
    
    ###### EDITED
    #tp_compute_total_throughput = num_generated_tokens / tp_compute_total_latency

    #tp_compute_throughput = num_generated_tokens / tp_compute_decode_latency
    ...
    ################# writing computation only throughput
    #print(f'copmute only throughput:{ total_throughput_only_compute}, tp _compute_throughput:{tp_compute_throughput}')
    
    ############# EDITED
    #print(f'\n\ntensor parallel compute only PREFILL THROUGHPUT: {tp_compute_prefill_throughput},\n compute only DECODE THROUGHPUT: {tp_compute_decode_throughput},\n comptute only TOTAL THROUGHPUT: {tp_compute_total_throughput}\n')
...
def add_parser_arguments(parser):
    ...
    ########## Edited
    parser.add_argument("--rank_path", type=str, default="/home/aktarafder/ranks",
        help="tensor parallel sliced tensors will be in rank 0/ rank 1/... ")
...
if __name__ == "__main__":
    ...
    try:
        user_input = int(input("Enter a number less than the available GPUs: "))
        if user_input <= num_gpus:
            print(f"Valid input: {user_input}, proceding tensor parallel execution.")
        else:
            print(f"Invalid input! The number should be less than {num_gpus}.")
            sys.exit(0)
    except ValueError:
        print("Invalid input! Please enter an integer.")
    
    print('debug info: compress cache default set false in mah_gen, some print comments are there.\n')

    world_size = user_input
    backend = "nccl"
    mp.spawn(init_processes, args=(backend, args, world_size), nprocs=world_size, join=True)
