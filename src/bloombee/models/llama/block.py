...
# from bloombee.flexgen_utils.utils import ExecutionEnv
...
from bloombee.models.llama.llama_config import get_llama_config,download_llama_weights
from bloombee.flexgen_utils.task import Task
from transformers import AutoTokenizer
import os
# import sys
# sys.path.insert(0,'..')
# sys.path.insert(0,'/flexgen_model_in_bloombee/src/bloombee/')
...
class OptimizedLlamaAttention(FLEX_LlamaAttention):
    ...
    def forward(
        ...
        return self.temp_hidden_states.val,  None, None # bloombee :attn_output, None, past_key_value
...
class OptimizedLlamaDecoderLayer(LlamaDecoderLayer):  # used in block_utils.py return config.block_class(config)
    def __init__(self, config: LlamaConfig,layer_id: int, env: ExecutionEnv, policy: Policy, weight_home: array_1d, path: str, ):
        ...
        #layer_idx only matters for KV caching, and we re-implement it in bloombee
