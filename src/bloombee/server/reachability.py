...
def validate_reachability(peer_id, wait_time: float = 7 * 60, retry_delay: float = 15) -> None:
    ...
    for attempt_no in range(math.floor(wait_time / retry_delay) + 1):
        try:
            ...
            if response["success"]:
                logger.info("Server is reachable from the Internet. It will appear at https://health.bloombee.dev soon")
                return
            ...
            logger.warning(f"Skipping reachability check because health.bloombee.dev is down: {repr(e)}")
            return
